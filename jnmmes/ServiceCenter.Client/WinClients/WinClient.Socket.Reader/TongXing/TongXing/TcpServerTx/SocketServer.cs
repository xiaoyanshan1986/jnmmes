       ...
       /// </summary>
        ...
        override public void StartEngine()
        {
            ...
            {           
                ...
                //IPClientPoint = (IPEndPoint)clientSocket.RemoteEndPoint;
            ...
            //while (true)
            //{
            //    readerData = new byte[1024];
            //    rev = clientSocket.Receive(readerData);
            //    strBarcode = Encoding.ASCII.GetString(readerData, 1, rev - 3);
            //    OnAcceptString(IPClientPoint, strBarcode); //触发文本收到的事件
            //}
        } 

        private void SocketAcceptAsyncEventArgs()
        {
        ...
        private void ReadCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    string str = System.Text.Encoding.Default.GetString(haveDate);
                    OnAcceptString(stateOne.IpEndPoint, str); //触发文本收到的事件
                    //MessageBox.Show(str);
        ...
        private void loginInitialization(object stateOne1)
        {
            ...
            while (true)//如果2秒中之内客户端没有根据登录信息回复，将自动关闭这个客户端
            {
                ...
                { 
                    ...
                    FileOperate(stateOne.IpEndPoint.ToString()+"无法收到客户端登录信息");//记录;
                    break; 
                }
            }
        }
