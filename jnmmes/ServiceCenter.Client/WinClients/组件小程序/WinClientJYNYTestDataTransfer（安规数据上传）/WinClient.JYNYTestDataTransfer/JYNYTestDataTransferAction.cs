        ...
        public void Execute(JYNYTestDeviceElement element, System.DateTime MaxTestDateTimeFromServer)
        {
            ...
            try
            {
                ...
                if (blTransferDataResult)
                {
                    ...
                    sbSql.AppendFormat("SELECT TOP 1 '{2}' AS EQUIPMENT_CODE,a.* FROM ({0}) a WHERE a.LOT_NUMBER<>''and a.TEST_TIME>'{1}'and a.TEST_RESULT='PASS' ORDER BY a.TEST_TIME desc",
                                        sql,
                ...
                if (dsJYNYTestData != null && dsJYNYTestData.Tables.Count > 0 && dsJYNYTestData.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dsJYNYTestData.Tables[0].Rows)
                    {
                        ...
                        strInsertSql = @"INSERT INTO [dbo].[ZWIP_VIR_TESTDATA]
                                                       ([EQUIPMENT_CODE]
                                                       ,[TEST_TIME]
                                                       ,[LOT_NUMBER]
                                                       ,[TEST_RESULT]
                        ...
                        String LOT_NUMBER = Convert.ToString(dr["LOT_NUMBER"]).ToUpper().TrimEnd('\n');
                        ...
                        strInsertSql = strInsertSql + "'" + Convert.ToString(dr["TEST_TYPE"]).ToUpper().Trim()+ "',";
                        ...
                        string TEST_ID = "";
                        TEST_ID = "select LOT_NUMBER from ZWIP_VIR_TESTDATA where LOT_NUMBER='" + dr["LOT_NUMBER"] + "'" + "and EQUIPMENT_CODE ='" + element.EqpName + "'" + "and TEST_TIME ='" + dr["TEST_TIME"] + "'" + "and TEST_TYPE ='" + dr["TEST_TYPE"] + "'";
                        dt = DatabaseEx.getDbInstance().getDataTable(TEST_ID);
                        if (TEST_ID =="")
                        {
