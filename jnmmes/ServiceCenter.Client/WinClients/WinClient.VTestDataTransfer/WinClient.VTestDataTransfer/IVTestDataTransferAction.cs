...
namespace WinClient.VTestDataTransfer
{
    ...
    public class IVTestDataTransferAction
    {
        ...
        /// <summary> 取得当前设备同步到数据库中的最后时间 </summary>
        /// <param name="element"></param>
        /// <returns></returns>
        private DateTime GetDBLastTestTime(IVTestDeviceElement element)
        {
            DateTime dtGetIVtestLastTime;
            ...
            //{
                ...
                sqlString = string.Format(@" select max(test_time) TestTime from [dbo].[ZWIP_IV_TEST] 
                                                where EQUIPMENT_CODE='{0}'", element.EqpName);
                ...
                {
                    dtGetIVtestLastTime = System.DateTime.Now;
                }

                //INIIVtestLastGetTime = true;

                return dtGetIVtestLastTime;
            //}
            //catch (Exception er)
            //{
        ...
        /// <summary> 将数据从Access数据库转移到MES数据库 </summary>
        /// <param name="element">设备配置。</param>
        /// <returns>true:转置成功。false：转置失败。</returns>,System.DateTime MaxTestDateTimeFromServer
...
//                                                 from [dbo].[ZWIP_IV_TEST] 
//                                                where EQUIPMENT_CODE='{0}'
//                                                      and LOT_NUMBER like 'JZ%' 
//                                                      and IS_DEFAULT =1 
                        ...
                        foreach (DataRow dr in dsIVTestData.Tables[0].Rows)
                        {
                            ...
                            if (!Convert.ToString(dr["LOT_NUMBER"]).ToUpper().StartsWith("JZ"))
                            {
                                ...
                                if (strCalibrationNo == "")
                                {
                                    ...
                                    if (dt.Rows.Count > 0)
                                    {
                                        strCalibrateTime = dr["TEST_TIME"].ToString();              //校准时间
                                        strCalibrationNo = dr["LOT_NUMBER"].ToString().ToUpper();   //校准板
                                    }                                    
                                }
                                ...
                                string sqlCTM = string.Format(@"SELECT t1.LOT_NUMBER ,
                                                                ...
                                                                FROM dbo.WIP_LOT AS t1
                                                                    INNER JOIN dbo.FMM_MATERIAL AS t2 
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        //取得控制区间
                                        double maxPM = (dt.Rows[0]["MAX_PM"] != DBNull.Value ? Convert.ToDouble(dt.Rows[0]["MAX_PM"]) : 0);     //最大功率
                                        double minPM = (dt.Rows[0]["MIN_PM"] != DBNull.Value ? Convert.ToDouble(dt.Rows[0]["MIN_PM"]) : 0);     //最小功率
                                        double maxISC = (dt.Rows[0]["MAX_ISC"] != DBNull.Value ? Convert.ToDouble(dt.Rows[0]["MAX_ISC"]) : 0);  //最大电流
                                        double minISC = (dt.Rows[0]["MIN_ISC"] != DBNull.Value ? Convert.ToDouble(dt.Rows[0]["MIN_ISC"]) : 0);  //最小电流
                                        double maxVOC = (dt.Rows[0]["MAX_VOC"] != DBNull.Value ? Convert.ToDouble(dt.Rows[0]["MAX_VOC"]) : 0);  //最大电压
                                        double minVOC = (dt.Rows[0]["MIN_VOC"] != DBNull.Value ? Convert.ToDouble(dt.Rows[0]["MIN_VOC"]) : 0);  //最小电压
                            ...
                            strInsertSql = @" INSERT INTO [dbo].[ZWIP_IV_TEST](
                                                    ...
                                                    ,[CREATOR],[CREATE_TIME],[EDITOR],[EDIT_TIME]) values (";
...
//                    //[Vpm], [Pmax], [Temp], [EnvTemp], [TMod], [Insol], [SunRef], [Test_Time] FROM SunData;
//                    sbSql.AppendFormat("SELECT TOP 1 '{2}' AS DeviceNo,a.* FROM ({0}) a WHERE a.TTIME>'{1}' ORDER BY a.TTIME ASC",
//                                        sql,
//                                        MaxTestDateTimeFromServer.ToString("yyyy-MM-dd HH:mm:ss"),
//                                        element.EqpName);
