...
namespace ServiceCenter.Client.Mvc.Areas.RPT.Controllers
{
  ...
  <BorderSkin SkinStyle=""Emboss"" />
        ...
        public ActionResult Query(WIPMoveQueryViewModel model)
        {
            ...
            DataTable dtDateForShifts = new DataTable();
            DataTable dtDateForShifts1 = new DataTable();
            DataTable dtDateForShifts2 = new DataTable();
            //获取工序MOVE数据。
            ...
            System.Data.DataView dView = new DataView(dtData);
            string[] columns = new string[] { "CUR_DAY", "SHIFT_NAME", "ACTIVITY" };
            string[] columns1 = new string[] { "CUR_DAY"};
            string[] columns2 = new string[] { "CUR_DAY",  "ACTIVITY" };
            dtDateForShifts = dView.ToTable(true, columns);
            dtDateForShifts1 = dView.ToTable(true, columns1);
            dtDateForShifts2 = dView.ToTable(true, columns2);
            //获取工序数据。
            ...
            foreach (DataRow row1 in dtDateForShifts1.Rows)
            {
                foreach (DataRow row2 in dtDateForShifts2.Rows)
                {
                    string t1 = row1["CUR_DAY"].ToString();
                    string t2 = row2["CUR_DAY"].ToString();
                    if (t1 == t2)
                    {
                        DataRow dr2 = dtNew.NewRow();
                        dr2["CUR_DAY"] = row2["CUR_DAY"];
                        dr2["SHIFT_NAME"] = "合计";
                        dr2["ACTIVITY"] = Convert.ToInt32(row2["ACTIVITY"]);
                        dtNew.Rows.Add(dr2);
                    }                    
                }
                foreach (DataRow row in dtDateForShifts.Rows)
                {
                    string t1 = row1["CUR_DAY"].ToString();
                    string t2 = row["CUR_DAY"].ToString();
                    if (t1 == t2)
                    {
                        DataRow dr = dtNew.NewRow();
                        dr["CUR_DAY"] = row["CUR_DAY"];
                        dr["SHIFT_NAME"] = row["SHIFT_NAME"];
                        dr["ACTIVITY"] = Convert.ToInt32(row["ACTIVITY"]);
                        dtNew.Rows.Add(dr);
                    }
                }
            }
