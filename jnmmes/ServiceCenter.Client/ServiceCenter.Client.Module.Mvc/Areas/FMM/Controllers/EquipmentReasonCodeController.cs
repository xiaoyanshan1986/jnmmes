...
{
    public class EquipmentReasonCodeController : Controller
    {
        //
        // GET: /FMM/ReasonCode/
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Save(EquipmentReasonCodeViewModel model)
        {
            using (EquipmentReasonCodeServiceClient client = new EquipmentReasonCodeServiceClient())
            {
                EquipmentReasonCode obj = new EquipmentReasonCode()
                {
                    Key = model.Name,
                    Type = model.Type,
                    Class=model.Class,
        ...
        {
            EquipmentReasonCodeViewModel viewModel = new EquipmentReasonCodeViewModel();
            using (EquipmentReasonCodeServiceClient client = new EquipmentReasonCodeServiceClient())
            {
                MethodReturnResult<EquipmentReasonCode> result = await client.GetAsync(key);
                if (result.Code == 0)
                {
                    viewModel = new EquipmentReasonCodeViewModel()
                    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> SaveModify(EquipmentReasonCodeViewModel model)
        {
            using (EquipmentReasonCodeServiceClient client = new EquipmentReasonCodeServiceClient())
            {
                MethodReturnResult<EquipmentReasonCode> result = await client.GetAsync(model.Name);
