...
{
    public class IVTestDataQueryController : Controller
    {
        ...
        /// <returns></returns>
        public ActionResult QueryIVdata(IVTestDataQueryViewModel model)
        {
            ...
            try
            {
                using (IVTestDataServiceClient client = new IVTestDataServiceClient())
                {
                    ...
                    //MinPaiData mmmm = client.GetMinPaiData(model.LotNumber, "103A-B");

                    //string sss = client.CallBack(model.LotNumber, "成功", "103A-B");

                    result = client.GetIVdata(ref param);
        ...
        public async Task<ActionResult> JzQuery(IVTestDataQueryViewModel model)
        {
            ...
            if (Request.IsAjaxRequest())
            {
                return PartialView("_JzListPartial", new IVTestDataViewModel());
            }
            else
            {
        ...
        public async Task<ActionResult> PagingQuery(string where, string orderBy, int? currentPageNo, int? currentPageSize)
        {
            ...
            {
                return View("JzIndex", new IVTestDataViewModel());
            }
            //return PartialView("_ListPartial", new IVTestDataViewModel());
        }
        //
        ...
        public async Task<ActionResult> ExportToExcelIVdata(IVTestDataQueryViewModel model)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    {
                        if (indexOfCol != 17 && indexOfCol != 18 && indexOfCol != 19 && indexOfCol != 20 && indexOfCol != 21)
                        {
        ...
        public ActionResult JzExportToExcel(IVTestDataQueryViewModel model)
        {
            ...
            for (int j = 0; j < lstIVTestData.Count; j++)
            {
                if (j % 65535 == 0)
                {
                    ...
                    cell = row.CreateCell(row.Cells.Count);
                    cell.CellStyle = style;
                    cell.SetCellValue("标准电流1");

                    cell = row.CreateCell(row.Cells.Count);
                    cell.CellStyle = style;
                    cell.SetCellValue("标准电流2");

                    cell = row.CreateCell(row.Cells.Count);
                    cell.CellStyle = style;
                    cell.SetCellValue("标准光强1");

                    cell = row.CreateCell(row.Cells.Count);
                    cell.CellStyle = style;
                    cell.SetCellValue("标准光强2");
                ...
                cellData = rowData.CreateCell(rowData.Cells.Count);
                cellData.CellStyle = style;
                cellData.SetCellValue(obj.StdIsc1);

                cellData = rowData.CreateCell(rowData.Cells.Count);
                cellData.CellStyle = style;
                cellData.SetCellValue(obj.StdIsc2);

                cellData = rowData.CreateCell(rowData.Cells.Count);
                cellData.CellStyle = style;
                cellData.SetCellValue(obj.Stdsun1);

                cellData = rowData.CreateCell(rowData.Cells.Count);
                cellData.CellStyle = style;
                cellData.SetCellValue(obj.Stdsun2);
        ...
        public string GetJzQueryCondition(IVTestDataQueryViewModel model)
        {
            ...
            if (localName == "G01")
            {
                where.Append("  IS_CALIBRATIONPLATE = 1  ");
            }
