...
namespace ServiceCenter.MES.Service.WIP.ServiceExtensions
{
    ...
    /// 扩展批次包装检查，进行工单包装规则检查。
    /// </summary>
    class LotPackageForCheckWorkOrderRule : ILotPackageCheck
    {
        /// <summary>
        /// 批次数据访问类。
        /// </summary>
        ...
        public MethodReturnResult Check(PackageParameter p)
        {
            ...
            if (packageObj != null && packageObj.Quantity>0)
            {
                ...
                {
                    result.Code = 3005;
                    result.Message = string.Format("该工单号（{0}）、物料号（{1}）、分档代码（{2}）和项目号（{3}）没有设置混颜色工单规则"
                                                    , lot.OrderNumber
                                                    , lot.MaterialCode
                                                    , packagePowersetCode
                                                    , packagePowersetCodeItemNo);
                    return result;
                }
                if (wop.MixColor)
                {
            ...
            foreach(string lotNumber in p.LotNumbers)
            {
                ...
                if (isCheckPackage)
                {
                    ...
                    {
                        result.Code = 3000;
                        result.Message = string.Format("批次子分档信息({0})不存在，无法进行包装。"
                                                     , powersetCode);
                        return result;
                    }
                    if(wop.MixColor)
