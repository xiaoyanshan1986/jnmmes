...
namespace ServiceCenter.MES.Service.WIP
{
    ...
    public partial class LotBinServiceEx : ILotBinContract,ILotBinCheck,ILotBin
    {
        ...
        ///  扔到24bin包装数据历史。
        /// </summary>
        public IPackageCornerDetailTransactionDataEngine PackageCornerDetailTransactionDataEngine
        {
            get;
            set;
        }
        ...
        string localName0 = System.Configuration.ConfigurationSettings.AppSettings["LocalName"];
        string packageType0 = System.Configuration.ConfigurationSettings.AppSettings["PackageType"];
        ...
        ///                 2.ScanIP        -- 读头IP地址
        ///                 3.BinNo         -- 入Bin号
        ///                 4.ScanNo        -- 组件的入Bin位置代码
        /// <returns>1.code         -- 执行代码 0 - 成功 其它 - 失败
        ///          2.Detail       -- 需要入bin号 
        ...
        MethodReturnResult ILotBinContract.InBin(InBinParameter p)
        {
            ...
            if (packageType0 == "30")
            {
                ...
                if (localName0 == "G01")
                {
                    LogHelper.WriteLogError(string.Format("InBin:Start> Lot_Number:{0}|| PackageLine:{1}||Bin_No:{2}", p.ScanLotNumber, p.PackageLine, p.BinNo));
                }
                ...
                try
                {
                    ...
                    {
                        ProductionLine productionLine = this.ProductionLineDataEngine.Get(p.PackageLine);//该BIN入异常BIN时直接返回，入异常托
                        if (productionLine != null && productionLine.Attr2 != "")
                        {
                            string abnormalBinNo = productionLine.Attr2;
                            if (abnormalBinNo == p.BinNo)
                            {
                                result = InAbnormalBin(p);
                                return result;
                            }
                        }
                        //判断需入Bin编号是否与校验相同
                        if (p.BinNo != result.Detail)
                        {
                            result.Code = 1000;
                            result.Message = string.Format("请求入Bin号：({0})与计算结果不符，计算应入（{1}）号Bin！",
                                                                p.BinNo,
                                                                result.Detail);
                        }
                        else
                        {
                            //判断入Bin位置信息是否相同
                            if (p.ScanNo != result.ObjectNo)
                            {
                                result.Code = 1000;
                                result.Message = string.Format("请求入Bin顺序：({0})与计算结果不符，计算应入顺序号：（{1}）！",
                                                                    p.ScanNo,
                                                                    result.ObjectNo);
                            }
                            else
                            {
                                ...
                                {
                                    //入Bin
                                    result = ExecuteInBinEx(p, int.Parse(result.ObjectNo));
        ...
        */
        ...
        MethodReturnResult ILotBin.Execute(InBinParameter p)
        {
            ...
            if (packageType0 == "30")
            {
                ...
                if (blCreatePackageFlag)
                {
                    ...
                    packageNo = resultPackageNo.Data;
        ...
        public MethodReturnResult<IList<PackageBin>> QueryBinListFromPackageLine(string packageLine)
        {
            ...
            {
                #region 全包护角
                result = new MethodReturnResult<IList<PackageBin>>
                {
                    Code = 0,
                };
                ...
                {
                    IsPaging = false,
                    Where = string.Format(" Key.PackageLine ='{0}' AND BinPackaged=0 and Key.PackageNo<>'' and Key.PackageNo is not null and BinState<>1", packageLine),
                    OrderBy = "Key.BinNo "
                };
