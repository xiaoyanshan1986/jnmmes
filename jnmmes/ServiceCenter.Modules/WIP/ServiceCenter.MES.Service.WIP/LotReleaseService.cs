...
namespace ServiceCenter.MES.Service.WIP
{
    ...
    public class LotReleaseService : ILotReleaseContract, ILotReleaseCheck, ILotRelease
    {
        /// <summary>
        /// 操作前检查事件。
        /// </summary>
        public event Func<ReleaseParameter, MethodReturnResult> CheckEvent;
        /// <summary>
        /// 执行操作时事件。
        /// </summary>
        public event Func<ReleaseParameter, MethodReturnResult> ExecutingEvent;
        /// <summary>
        /// 操作执行完成事件。
        /// </summary>
        public event Func<ReleaseParameter, MethodReturnResult> ExecutedEvent;

        /// <summary>
        /// 自定义操作前检查的清单列表。
        /// </summary>
        private IList<ILotReleaseCheck> CheckList { get; set; }
        /// <summary>
        /// 自定义执行中操作的清单列表。
        /// </summary>
        private IList<ILotRelease> ExecutingList { get; set; }
        /// <summary>
        /// 自定义执行后操作的清单列表。
        /// </summary>
        private IList<ILotRelease> ExecutedList { get; set; }
        ...
        public void RegisterCheckInstance(ILotReleaseCheck obj)
        {
            if (this.CheckList == null)
            {
                this.CheckList = new List<ILotReleaseCheck>();
            }
        ...
        /// </summary>
        /// <param name="obj"></param>
        public void RegisterExecutingInstance(ILotRelease obj)
        {
            if (this.ExecutingList == null)
            {
                this.ExecutingList = new List<ILotRelease>();
            }
        ...
        /// </summary>
        /// <param name="obj"></param>
        public void RegisterExecutedInstance(ILotRelease obj)
        {
            if (this.ExecutedList == null)
            {
                this.ExecutedList = new List<ILotRelease>();
            }
        ...
        /// <param name="p"></param>
        /// <returns></returns>
        protected virtual MethodReturnResult OnCheck(ReleaseParameter p)
        {
            ...
            {
                foreach (Func<ReleaseParameter, MethodReturnResult> d in this.CheckEvent.GetInvocationList())
                {
                    result = d(p);
                    if (result.Code > 0)
                    {
            ...
            if (this.CheckList != null)
            {
                foreach (ILotReleaseCheck d in this.CheckList)
                {
                    result = d.Check(p);
                    if (result.Code > 0)
                    {
        ...
        /// 操作执行中。
        /// </summary>
        protected virtual MethodReturnResult OnExecuting(ReleaseParameter p)
        {
            MethodReturnResult result = new MethodReturnResult()
            {
                Code = 0
            ...
            {
                foreach (Func<ReleaseParameter, MethodReturnResult> d in this.ExecutingEvent.GetInvocationList())
                {
                    result = d(p);
                    if (result.Code > 0)
                    {
            ...
            if (this.ExecutingList != null)
            {
                foreach (ILotRelease d in this.ExecutingList)
                {
                    result = d.Execute(p);
                    if (result.Code > 0)
                    {
        ...
        /// 执行完成。
        /// </summary>
        protected virtual MethodReturnResult OnExecuted(ReleaseParameter p)
        {
            MethodReturnResult result = new MethodReturnResult()
            {
                Code = 0
            ...
            {
                foreach (Func<ReleaseParameter, MethodReturnResult> d in this.ExecutedEvent.GetInvocationList())
                {
                    result = d(p);
                    if (result.Code > 0)
                    {
            ...
            if (this.ExecutedList != null)
            {
                foreach (ILotRelease d in this.ExecutedList)
                {
                    result = d.Execute(p);
                    if (result.Code > 0)
                    {
        ...
        /// 代码表示：0：成功，其他失败。
        /// </returns>
        MethodReturnResult ILotReleaseContract.Release(ReleaseParameter p)
        {
            MethodReturnResult result = new MethodReturnResult();
            if (p == null)
            {
        ...
        /// <param name="p"></param>
        /// <returns></returns>
        MethodReturnResult ILotReleaseCheck.Check(ReleaseParameter p)
        {
            MethodReturnResult result = new MethodReturnResult()
            {
                Code = 0
        ...
        /// <param name="p"></param>
        /// <returns></returns>
        MethodReturnResult ILotRelease.Execute(ReleaseParameter p)
        {
            ...
            foreach(string lotNumber in p.LotNumbers)
            {
                ...
                {
                    IsPaging = false,
                    Where = string.Format(@"EXISTS (SELECT Key FROM LotTransaction as p
                                                    WHERE p.Key=self.Key
                                                    AND p.LotNumber='{0}'
                                                    AND p.Activity='{1}')
                                            AND IsRelease=0"
                                           , lotNumber
                                           , Convert.ToInt32(EnumLotActivity.Hold))
                };
                IList<LotTransactionHoldRelease> lstHoldRelease = this.LotTransactionHoldReleaseDataEngine.Get(cfg);
                if (lstHoldRelease.Count > 0)
                {
                    //进行密码验证。
                    var lnq = from item in lstHoldRelease
                              where string.IsNullOrEmpty(item.HoldPassword) || item.HoldPassword == p.ReleasePassword
                              select item;
                    if (lnq.Count() == 0)
                    {
                        result.Code = 1004;
                        result.Message = string.Format("批次（{0}）释放密码错误。", lotNumber);
                        return result;
                    }
                }
                //生成操作事务主键。
                ...
                {
                    Key =  transaciontKey,
                    Activity = EnumLotActivity.Release,
                    CreateTime =now,
                    Creator=p.Creator,
                ...
                //更新暂停记录。
                foreach(LotTransactionHoldRelease item in lstHoldRelease)
                {
                    LotTransactionHoldRelease itemUpdate = item.Clone() as LotTransactionHoldRelease;
                    itemUpdate.ReleaseTransactionKey = transaciontKey;
                    itemUpdate.ReleaseTime = now;
                    itemUpdate.ReleaseOperator = p.Operator;
                    itemUpdate.ReleaseDescription = p.ReleaseDescription;
                    itemUpdate.IsRelease = true;
                    itemUpdate.Editor = p.Creator;
                    itemUpdate.EditTime = now;
                    this.LotTransactionHoldReleaseDataEngine.Update(itemUpdate);
                };
                #endregion
