...
namespace ServiceCenter.MES.Service.WIP
{
    ...
    {
        string ErpORGCode = System.Configuration.ConfigurationSettings.AppSettings["LocalName"];
        ...
        public MethodReturnResult ExecuteTrackInLot(string workModel, TrackInParameter p, Equipment equipment, List<Lot> lstLot, ISession session, bool executedWithTransaction)
        {
            ...
            DateTime startTransTime = DateTime.Now;             //事物开始时间
            DateTime endTransTime = DateTime.Now;               //事物结束时间
            string endtransactionKey = "";
            ...
            try
            {
                ...
                foreach (Lot lot in lstLot)
                {
                    ...
                    //1.记录批次历史状态( WIP_TRANSACTION_LOT )
                ...
                if (isEquipmentChangeState == true)
                {
                    endTransTime = DateTime.Now;                    

                    if (isAutoTrackOut)
                    {
                        endTransTime = endTransTime.AddMilliseconds(10);

                        if (p.TransactionKeys != null && p.TransactionKeys.Count > 0)
                        {
                            endtransactionKey = p.TransactionKeys[lstLot[0].Key];
                        }
                    }

                    //新增设备状态事件数据                    
                    result = EquipmentChangeState(equipment, equipmentBack.StateName, session, transactionKey, endtransactionKey, true, startTransTime, endTransTime);
                }
                ...
                foreach (LotTransactionEquipment lotTransactionEquipment in lstLotTransactionEquipmentDataEngineForInsert)
                {
                    if (isAutoTrackOut)
                    {
                        endTransTime = endTransTime.AddMilliseconds(10);                //事物结束时间

                        //取得结束事物主键（出站事物主键）
                        if (p.TransactionKeys != null && p.TransactionKeys.Count > 0)
                        {
                            endtransactionKey = p.TransactionKeys[lotTransactionEquipment.LotNumber];
                        }

                        lotTransactionEquipment.EndTransactionKey = endtransactionKey;          //结束事物主键
                        lotTransactionEquipment.EndTime = endTransTime;                         //结束事物时间
                        lotTransactionEquipment.State = EnumLotTransactionEquipmentState.End;   //事物结束状态
                        lotTransactionEquipment.EditTime = endTransTime;                        //编辑时间
                    }
                ...
                foreach (Lot lot in lstLot)
                {
                    ...
                    if (isCheck == true)
                    {
                        ...
                        for (int i = 0; i < lstTestType.Length; i++)
                        {
                            ...
                            {
                                if (virTestData.Key.TestType == lstTestType[i] && virTestData.TestResult == "PASS")
                                {
                                    isPass = true;
            ...
            try
            {
                ...
                foreach (Lot lot in lstLot)
                {
                    ...
                    if (isCheck == true)
                    {
                        ...
                        #region //判断IV测试数据RSH是否满足条件
                        PagingConfig cfg0 = new PagingConfig()
                        {
                            Where = string.Format("Key.Object='8' AND Key.Type='>' AND Key.OrderNumber='{0}' AND IsUsed=1",lot.OrderNumber),
                            IsPaging = false
                        };
                        IList<WorkOrderControlObject> lstWorkOrderControlObjectData=this.WorkOrderControlObjectDataEngine.Get(cfg0);
                        PagingConfig cfg1 = new PagingConfig()
                        {
                            Where = string.Format("Key.Object='8' AND Key.Type='>=' AND Key.OrderNumber='{0}' AND IsUsed=1",lot.OrderNumber),
                            IsPaging = false
                        };
                        IList<WorkOrderControlObject> lstWorkOrderControlObjectData1=this.WorkOrderControlObjectDataEngine.Get(cfg1);
                        PagingConfig cfg2 = new PagingConfig()
                        {
                            Where = string.Format("Key.Object='8' AND Key.Type='=' AND Key.OrderNumber='{0}' AND IsUsed=1",lot.OrderNumber),
                            IsPaging = false
                        };
                        IList<WorkOrderControlObject> lstWorkOrderControlObjectData2=this.WorkOrderControlObjectDataEngine.Get(cfg2);
                        PagingConfig cfg3 = new PagingConfig()
                        {
                            Where = string.Format("Key.Object='8' AND Key.Type='<' AND Key.OrderNumber='{0}' AND IsUsed=1",lot.OrderNumber),
                            IsPaging = false
                        };
                        IList<WorkOrderControlObject> lstWorkOrderControlObjectData3=this.WorkOrderControlObjectDataEngine.Get(cfg3);
                        PagingConfig cfg4 = new PagingConfig()
                        {
                            Where = string.Format("Key.Object='8' AND Key.Type='<=' AND Key.OrderNumber='{0}' AND IsUsed=1",lot.OrderNumber),
                            IsPaging = false
                        };
                        IList<WorkOrderControlObject> lstWorkOrderControlObjectData4=this.WorkOrderControlObjectDataEngine.Get(cfg4);
                        PagingConfig cfg5 = new PagingConfig()
                        {
                            Where = string.Format("Key.Object='8' AND Key.Type='<>' AND Key.OrderNumber='{0}' AND IsUsed=1",lot.OrderNumber),
                            IsPaging = false
                        };
                        IList<WorkOrderControlObject> lstWorkOrderControlObjectData5=this.WorkOrderControlObjectDataEngine.Get(cfg5);
                        //判断IV测试数据RSH是否满足条件
                        if(lstWorkOrderControlObjectData5.Count>0)
                        {
                            if (Convert.ToDouble(lstTestData[0].RSH).ToString("0.00") == Convert.ToDouble(lstWorkOrderControlObjectData5[0].Value).ToString("0.00"))
                            {
                                result.Code = 2200;
                                result.Message = string.Format("批次（{0}）在IV测试数据中RSH值不可等于{1}，请重新测试。", lot.Key, Convert.ToDouble(lstWorkOrderControlObjectData5[0].Value).ToString("0.00"));
                                return result;
                            }                                                 
                        }
                        if (lstWorkOrderControlObjectData2.Count > 0)
                        {
                            if (Convert.ToDouble(lstTestData[0].RSH).ToString("0.00") != Convert.ToDouble(lstWorkOrderControlObjectData2[0].Value).ToString("0.00"))
                            {
                                result.Code = 2200;
                                result.Message = string.Format("批次（{0}）在IV测试数据中RSH值必须等于{1}，请重新测试。", lot.Key, Convert.ToDouble(lstWorkOrderControlObjectData2[0].Value).ToString("0.00"));
                                return result;
                            }
                        }
                        if (lstWorkOrderControlObjectData.Count > 0)
                        {
                            if (Convert.ToDouble(lstTestData[0].RSH) <= Convert.ToDouble(lstWorkOrderControlObjectData[0].Value))
                            {
                                result.Code = 2200;
                                result.Message = string.Format("批次（{0}）在IV测试数据中RSH值必须大于{1}，请重新测试。", lot.Key, Convert.ToDouble(lstWorkOrderControlObjectData[0].Value).ToString("0.00"));
                                return result;
                            }
                        }
                        if (lstWorkOrderControlObjectData1.Count > 0)
                        {
                            if (Convert.ToDouble(lstTestData[0].RSH) < Convert.ToDouble(lstWorkOrderControlObjectData1[0].Value))
                            {
                                result.Code = 2200;
                                result.Message = string.Format("批次（{0}）在IV测试数据中RSH值必须大于等于{1}，请重新测试。", lot.Key, Convert.ToDouble(lstWorkOrderControlObjectData1[0].Value).ToString("0.00"));
                                return result;
                            }
                        }
                        if (lstWorkOrderControlObjectData3.Count > 0)
                        {
                            if (Convert.ToDouble(lstTestData[0].RSH) >= Convert.ToDouble(lstWorkOrderControlObjectData3[0].Value))
                            {
                                result.Code = 2200;
                                result.Message = string.Format("批次（{0}）在IV测试数据中RSH值必须小于{1}，请重新测试。", lot.Key, Convert.ToDouble(lstWorkOrderControlObjectData3[0].Value).ToString("0.00"));
                                return result;
                            }
                        }
                        if (lstWorkOrderControlObjectData4.Count > 0)
                        {
                            if (Convert.ToDouble(lstTestData[0].RSH) > Convert.ToDouble(lstWorkOrderControlObjectData4[0].Value))
                            {
                                result.Code = 2200;
                                result.Message = string.Format("批次（{0}）在IV测试数据中RSH值必须小于等于{1}，请重新测试。", lot.Key, Convert.ToDouble(lstWorkOrderControlObjectData4[0].Value).ToString("0.00"));
                                return result;
                            }
                        }
                        #endregion
                        ...
                        if (lstTestData[0].CalibrationNo.ToUpper().Length >= 11)
                        {
        ...
        {
            DateTime now = DateTime.Now.AddMilliseconds(20);
            DateTime startTransTime = DateTime.Now;
            DateTime endTransTime = DateTime.Now;
            ...
            try
            {
                ...
                if (isEquipmentChangeState == true)
                {
                    ...
                    {
                        //新增设备状态事件数据                    
                        result = EquipmentChangeState(equipment, sFromStateName, session, transactionKey, "", false, startTransTime, endTransTime);
                    }
                    else
                    {
                        //新增设备状态事件数据                    
                        result = EquipmentChangeState(equipment, sFromStateName, session, transactionKey, "", true, startTransTime, endTransTime);
                    }
                }
                ...
                {
                    lotTransactionEquipmentUpdate.EditTime = endTransTime;
                    lotTransactionEquipmentUpdate.EndTime = endTransTime;
        ...
        /// <param name="endTransactionKey">结束业务事物主键</param>
        /// <param name="isGetPrevious">是否取得历史状态</param>
        /// <param name="startTransTime">开始时间</param>
        /// <param name="endTransTime">结束时间</param>
        /// <returns></returns>
        public MethodReturnResult EquipmentChangeState(Equipment equipment, string sFromStateName, ISession session, string transactionKey, string endTransactionKey, bool isGetPrevious, DateTime startTransTime, DateTime endTransTime)
        {
            MethodReturnResult result = new MethodReturnResult();
            ...
            try
            {
                if (isGetPrevious == true)
                {
                    ...
                    if (lsEquipmentStateEvent != null && lsEquipmentStateEvent.Count > 0)
                    {
                        ...
                        lsEquipmentStateEvent[0].EndTime = startTransTime;              //事件结束时间
                        lsEquipmentStateEvent[0].EditTime = startTransTime;             //编辑时间
                ...
                EquipmentStateEvent newEquipmentStateEvent = new EquipmentStateEvent()
                {
                    ...
                    StartTime = startTransTime,                                     //事件开始时间
                    EndTime = endTransTime,                                         //事件结束时间
                    EndEventKey = endTransactionKey,                               //结束事物
                    Description = "",                                               //描述
                    IsCurrent = true,                                               //当前记录
                ...
                if (endTransactionKey != "")
                {
                    newEquipmentStateEvent.IsCurrent = false;
                }
        ...
        private MethodReturnResult ProductLevelPower(IVTestData ivtestDescy, Lot lot)
        {
            ...
            try
            {
                ...
                //AND (MaterialCode like'11%' OR MaterialCode like'18%')"
                ...
                {
                    foreach (LotBOM item in lotBomList)
                    {
                        Material material = this.MaterialDataEngine.Get(item.MaterialCode.ToString());
                        if (material != null && material.Name.IndexOf("电池") > 0)
                        {
                            materialCode = material.Key;
                            break;
                        }
                    }
                    cfg.Where = string.Format(@"Key.LotNumber='{0}' AND MaterialCode = '{1}'"
                                               , lot.Key
                                               , materialCode);

                    lotBomList = this.LotBOMDataEngine.Get(cfg);
                ...
                {
                    result.Code = 2000;
                    result.Message = string.Format("批次{0}用料中不存在包含《电池》的用料名称,请核实!", lot.Key);
                ...
                if (ps.SubWay != EnumPowersetSubWay.None)
                {
                    ...
                    {
                        if (lot.MaterialCode == "1201020047")
                        {
                            result.Message += string.Format("\r\n批次（{0}）符合工单（{1}:{2}）分档规则<font size='20' color='red'>({3})</font>要求。<font size='20' color='red'>电池片效率为：{4}</font>"
                            , lot.Key
                            , lot.OrderNumber
                            , lot.MaterialCode
        ...
        private MethodReturnResult IVTestDescy(IVTestData ivtest, Lot lot)
        {
            ...
            try
            {
                ...
                if (lot.MaterialCode == "1202020012")
                {
                    ...
                    {
                        if (ivtest.PM > 316 && ivtest.PM < 319.99999)
                        {
                            double rate = 0.997;
                            ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                            ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                        ...
                        {
                            if (ivtest.PM > 320.5 && ivtest.PM < 320.9999)
                            {
                                double rate = 0.9985;
                                ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                                ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                            ...
                            {
                                if (ivtest.PM > 321 && ivtest.PM < 325.99999)
                                {
                                    double rate = 0.997;
                                    ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                                    ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                ...
                if (lot.MaterialCode == "1202020020")
                {
                    ...
                    {
                        if (ivtest.PM > 321 && ivtest.PM < 325)
                        {
                            double rate = 0.997;
                            ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                            ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                        ...
                        {
                            if (ivtest.PM > 325 && ivtest.PM < 327)
                            {
                                double rate = 0.994;
                                ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                                ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                ...
                if (lot.MaterialCode == "1202020013")
                {
                    ...
                    {
                        if (ivtest.PM > 271 && ivtest.PM < 275)
                        {
                            double rate = 0.997;
                            ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                            ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                        ...
                        else
                        {
                            ...
                            if (ivtest.PM > 275.5 && ivtest.PM < 276)
                            {
                                double rate = 0.9985;
                                ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                                ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                            ...
                            else
                            {
                                ...
                                if (ivtest.PM > 276 && ivtest.PM < 280)
                                {
                                    double rate = 0.997;
                                    ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                                    ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                ...
                #region 1201020011料号衰减
                if (lot.MaterialCode == "1201020011")
                {
                    //实测功率在305---307的组件对其Pmax、Voc、Vpm都乘以0.997
                    if (ivtest.PM > 305 && ivtest.PM <= 307)
                    {
                        double rate = 0.997;
                        ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                        ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                    ...
                    //实测功率在307以上的组件对其Pmax、Voc、Vpm都乘以0.995
                    else
                    {
                        if (ivtest.PM > 307)
                        {
                ...
                #region 1201020047料号衰减
                if (lot.MaterialCode == "1201020047")
                {
                    if (ivtest.PM > 305 && ivtest.PM <= 306)
                    {
                        double rate = 0.996;
                        ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                        ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                    ...
                    else if (ivtest.PM > 306 && ivtest.PM <= 307)
                    {
                        double rate = 0.993;
                        ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                        ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                    ...
                    else if (ivtest.PM > 307 && ivtest.PM <= 308)
                    {
                        double rate = 0.99;
                        ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                        ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                ...
                #region 1202020104料号衰减
                if (lot.MaterialCode == "1202020104")
                {
                    if (ivtest.PM > 335 && ivtest.PM <= 336)
                    {
                        double rate = 0.997;
                        ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                        ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                ...
                #region 晋中料号衰减(1201020067/1201020071/1201020085)
                if (ErpORGCode == "G01")
                {
                    if (lot.MaterialCode == "1201020067")
                    {
                        //实测功率在315--316的组件对其Pmax、Voc、Vpm都乘以0.996
                        if (ivtest.PM > 315 && ivtest.PM <= 316)
                        {
                            double rate = 0.996;
                            ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                            ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                    ...
                    if (lot.MaterialCode == "1201020071")
                    {
                        //实测功率在370--372的组件对其Pmax、Voc、Vpm都乘以0.993
                        if (ivtest.PM > 370 && ivtest.PM <= 372)
                        {
                            double rate = 0.993;
                            ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                            ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                    ...
                    if (lot.MaterialCode == "1201020085")
                    {
                        //实测功率在376--380.99的组件对其Pmax、Voc、Vpm都乘以0.987
                        if (ivtest.PM >= 376 && ivtest.PM < 381)
                        {
                            double rate = 0.9974;
                            ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                            ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                        ...
                        //实测功率在381--381.99的组件对其Pmax、Voc、Vpm都乘以0.987
                        else if (ivtest.PM >= 381 && ivtest.PM < 382)
                        {
                            double rate = 0.9947;
                            ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                            ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                        ...
                        //实测功率在382--382.99的组件对其Pmax、Voc、Vpm都乘以0.987
                        else if (ivtest.PM >= 382 && ivtest.PM < 383)
                        {
                            double rate = 0.9921;
                            ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                            ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                        ...
                        //实测功率在383--383.99的组件对其Pmax、Voc、Vpm都乘以0.987
                        else if (ivtest.PM >= 383 && ivtest.PM < 384)
                        {
                            double rate = 0.9896;
                            ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                            ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                        ...
                        //实测功率在384--384.99的组件对其Pmax、Voc、Vpm都乘以0.987
                        else if (ivtest.PM >= 384 && ivtest.PM < 385)
                        {
                            double rate = 0.9870;
                            ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                            ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                ...
                #region 1201020075料号衰减
                if (lot.MaterialCode == "1201020075")
                {
                    if (ivtest.PM > 370 && ivtest.PM <= 371)
                    {
                        double rate = 0.997;
                        ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                        ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                    ...
                    else if (ivtest.PM > 371 && ivtest.PM <= 372)
                    {
                        double rate = 0.994;
                        ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                        ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                    ...
                    else if (ivtest.PM > 372 && ivtest.PM <= 373)
                    {
                        double rate = 0.991;
                        ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                        ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                    ...
                    else if (ivtest.PM > 373 && ivtest.PM <= 373.5)
                    {
                        double rate = 0.99;
                        ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                        ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                ...
                #region 1201020079/1201020072料号衰减
                if (lot.MaterialCode == "1201020079")
                {
                    //if (ivtest.CoefPM >= 363 && ivtest.CoefPM < 365)
                    //{
                    //    double rate = 1.007;
                    //    ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                    //    ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                    //    ivtest.CoefVPM = Math.Round(ivtest.VPM * rate, 4);
                    //}  
                    if (ivtest.PM >= 366 && ivtest.PM < 367)
                    {
                        double rate = 0.998;
                        ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                        ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                    ...
                    else if (ivtest.PM >= 367 && ivtest.PM < 370)
                    {
                        double rate = 0.997;
                        ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                        ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                    ...
                    else if (ivtest.PM >= 372 && ivtest.PM < 375)
                    {
                        double rate = 0.997;
                        ivtest.CoefPM = Math.Round(ivtest.PM * rate, 4);
                        ivtest.CoefVOC = Math.Round(ivtest.VOC * rate, 4);
                                ...
                                cfg = new PagingConfig()
                                {
                                    ...
                                    Where = string.Format(@"OrderNumber = '{0}'
                                                        ...
                                                        AND EXISTS(SELECT Key
                                                                    ...
                                                                    AND ((p.StartLot<='{4}'
                                                                    AND p.EndLot>='{4}')
                                                                    OR  (p.StartLot is null and p.EndLot is null))
        ...
        /// <summary> 取得物料BOM消耗量\最小扣料量、物料代码 </summary>
